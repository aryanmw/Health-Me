...
public class DayThursdayWorkout extends AppCompatActivity implements PersonalisedWorkoutDialogBoxListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_day_thursday_workout);
        myListView1 = (ListView) findViewById(R.id.MonListView);

        myList = new ArrayList<>();

        Cursor data = myDb.getThursdayData();
    ...
    @Override
    public void doWork() {
        myDb.removeRecord(workoutName,"Thursday");
        Cursor wData = myDb.getThursdayData();
        myList = new ArrayList<>();
        if(wData.getCount() == 0){
