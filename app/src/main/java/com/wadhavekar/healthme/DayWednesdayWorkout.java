...
public class DayWednesdayWorkout extends AppCompatActivity implements PersonalisedWorkoutDialogBoxListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_day_wednesday_workout);
        myListView1 = (ListView) findViewById(R.id.MonListView);

        myList = new ArrayList<>();

        Cursor data = myDb.getWednesdayData();
    ...
    @Override
    public void doWork() {
        myDb.removeRecord(workoutName,"Wednesday");
        Cursor wData = myDb.getWednesdayData();
        myList = new ArrayList<>();
        if(wData.getCount() == 0){
