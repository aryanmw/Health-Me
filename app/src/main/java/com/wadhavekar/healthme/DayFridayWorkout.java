...
public class DayFridayWorkout extends AppCompatActivity implements PersonalisedWorkoutDialogBoxListener {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_day_friday_workout);
        myListView1 = (ListView) findViewById(R.id.MonListView);

        myList = new ArrayList<>();

        Cursor data = myDb.getFridayData();
    ...
    @Override
    public void doWork() {
        myDb.removeRecord(workoutName,"Friday");
        Cursor wData = myDb.getFridayData();
        myList = new ArrayList<>();
        if(wData.getCount() == 0){
