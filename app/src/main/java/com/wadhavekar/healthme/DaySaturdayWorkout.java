...
public class DaySaturdayWorkout extends AppCompatActivity implements PersonalisedWorkoutDialogBoxListener{
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_day_saturday_workout);
        myListView1 = (ListView) findViewById(R.id.MonListView);

        myList = new ArrayList<>();

        Cursor data = myDb.getSaturdayData();
    ...
    @Override
    public void doWork() {
        myDb.removeRecord(workoutName,"Saturday");
        Cursor wData = myDb.getSaturdayData();
        myList = new ArrayList<>();
        if(wData.getCount() == 0){
